...
auto ArangoBench::createData(jsoncons::json& json) -> bool {
  ...
  for (auto at = 0; at < document_collections_.size(); at++) {
    ...
    ProgressLine bar(fmt::format("{} ({}/{}) - {} documents", collection.name(), at + 1, document_collections_.size(), count), count);
  ...
  for (auto at = 0; at < graphs.size(); at++) {
    ...
    ProgressLine bar(fmt::format("{} ({}/{}) - {} documents", name, at + 1, graphs.size(), count), count);
